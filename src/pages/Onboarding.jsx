...
const Onboarding = () => {
  ...
  const fetchEmployees = async () => {
    try {
      const response = await fetch(
        "https://hrmbackend.teqova.biz/api/getemployees"
      );
  ...
  const handleAddEmployee = async () => {
    try {
      ...
      const url = "https://hrmbackend.teqova.biz/api/adduser";
  ...
  const handleRemoveEmployee = async (e) => {
    ...
    //   const url = "https://hrmbackend.teqova.biz/api/deleteemployee";
    //   const response = await fetch(url, {
    //     method: "DELETE",
    //     headers: {
    //       "Content-Type": "application/json",
    ...
    <div className="flex h-screen">
      ...
      <div className="flex-1 overflow-auto ">
        <div className="p-4 bg-white shadow-md flex justify-between items-center" style={{ backgroundImage: `url(${navlogo})` }}>
          ...
          <h1 className="text-base md:text-xl font-bold text-white">{activeModule}</h1>
        </div>
