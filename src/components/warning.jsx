...
const WarningModule = ({ warningsData, actionData }) => {
  ...
  async function handleDownload(id, attachments) {
    try {
      ...
      const url = `http://hrmbackend.teqova.biz/api/download/${fileName}`;
  ...
  const handleAppeal = async (warningId) => {
    if (confirm("Confirm Case Closure !!")) {
      try {
        ...
        const url = "http://hrmbackend.teqova.biz/api/deletewarning";
        const response = await fetch(url, {
          method: "DELETE",
          headers: {
        ...
        if (result.message === "warning deleted") {
          const url2 = "http://hrmbackend.teqova.biz/api/warnings";
